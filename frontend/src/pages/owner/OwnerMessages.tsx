...
export default function OwnerMessages() {
    ...
    async function load() {
      ...
      try {
        ...
        if (!ignore) {
          ...
          if (groupParam) {
            const g = groupItems.find((gi) => gi.groupId === groupParam)
            if (g) {
              setActiveThread({ kind: 'group', groupId: g.groupId, groupName: g.groupName })
            } else {
              setActiveThread({ kind: 'group', groupId: groupParam, groupName: '' })
            }
            setSearchParams({}, { replace: true })
          } else if (tenantParam && apartmentParam) {
            const c = dmItems.find((ci) => ci.otherUserId === tenantParam && ci.apartmentId === apartmentParam)
            if (c) {
              setActiveThread({ kind: 'dm', apartmentId: c.apartmentId, otherUserId: c.otherUserId, otherUserName: c.otherUserName, apartmentTitle: c.apartmentTitle })
            } else {
              setActiveThread({ kind: 'dm', apartmentId: apartmentParam, otherUserId: tenantParam, otherUserName: '', apartmentTitle: '' })
            }
            setSearchParams({}, { replace: true })
          }
        }
    ...
    async function loadThread() {
      ...
      try {
        if (activeThread!.kind === 'dm') {
          const thread = activeThread as ActiveDMThread
          ...
          const thread = activeThread as ActiveGroupThread
          const msgs = await listGroupMessages(thread.groupId, 'owner')
          if (!ignore) {
  ...
  async function handleSendReply() {
    ...
    try {
      if (activeThread.kind === 'dm') {
        ...
        const newMsg = await sendGroupMessage(activeThread.groupId, trimmed, 'owner')
        setReplyText('')
  ...
  const sortedItems = useMemo(() => {
    ...
    const groupItems = groupConversations.map((g) => ({
      ...
      kind: 'group' as const,
      name: g.groupName,
      subtitle: g.lastSenderName ? `${g.lastSenderName}` : t('ownerDashboard.messages.groupChat'),
  ...
  const threadSubtitle = activeThread?.kind === 'dm' ? activeThread.apartmentTitle : t('ownerDashboard.messages.groupChat')
